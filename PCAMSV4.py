...
def center(DataMatrix):
    n,p = DataMatrix.shape
    DataCentered = DataMatrix
    m = np.mean(DataMatrix, axis = 1)
    for i in range(0, p):
        DataCentered[:,i] = DataCentered[:,i] - m
    return DataCentered

# Returns centered and standardized by row deviation data
def stdizerow(DataMatrix):
    n,p = DataMatrix.shape
    DataStandardized = center(DataMatrix)
    sig = np.std(DataMatrix, axis = 1, ddof = 1)
    for i in range(0,p):
        DataStandardized[:,i] /= sig
    return DataStandardized

# Returns centered and standardized by column deviation data
def stdizecol(DataMatrix):
    n,p = DataMatrix.shape
    sig = np.std(DataMatrix, axis = 0, ddof = 1)
    DataStandardized = center(DataMatrix)
    for i in range(0,n):
        DataStandardized[i,:] /= sig
    return DataStandardized
...
def normspectra(DataMatrix):
    n,p = DataMatrix.shape
    DataNormalized = DataMatrix
    maximums = np.amax(DataNormalized, axis = 0)
    for i in range(0,n):
        DataNormalized[i,:] /= maximums
    return DataNormalized
# Normalizes each sample by the sum of all features
def totionnorm(DataMatrix):
    n,p = DataMatrix.shape
    DataNormalized = DataMatrix
    totcurrents = np.sum(DataMatrix, axis = 0)
    for i in range(0,n):
        DataNormalized[i,:] /= totcurrents
    return DataNormalized
...
def ionsqnorm(DataMatrix):
    n,p = DataMatrix.shape
    DataNormalized = DataMatrix
    currentsq = np.sum(DataMatrix**2, axis = 0)
    for i in range(0,n):
        DataNormalized[i,:] /= currentsq
    return DataNormalized
...
# Need to have library - peakutils 
'''
